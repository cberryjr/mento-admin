...
export async function addQuoteSection(
  ...
  try {
    ...
    const newSection: QuoteSectionRecord = {
      id: newSectionId,
      quoteId: quote.id,
    ...
    const sections = recalculateQuoteTotals([...existingSections, newSection]);
    ...
    if (!updatedQuote.ok) {
      return {
        ...
        error: {
          code: ERROR_CODES.UNKNOWN,
          message: "Section was added but quote could not be reloaded.",
        },
      };
    }
