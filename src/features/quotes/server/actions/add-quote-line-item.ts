...
export async function addQuoteLineItem(
  ...
  try {
    ...
    const sections = recalculateQuoteTotals(
      existingSections.map((section, sIndex) => {
        ...
        const newLineItem = {
          ...
          unitLabel: "",
          unitPriceCents: 0,
          lineTotalCents: calculateLineTotalCents(1, 0),
          position: section.lineItems.length + 1,
        };
    ...
    if (!updatedQuote.ok) {
      return {
        ...
        error: {
          code: ERROR_CODES.UNKNOWN,
          message: "Line item was added but quote could not be reloaded.",
        },
      };
    }
