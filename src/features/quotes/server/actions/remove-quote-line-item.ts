...
export async function removeQuoteLineItem(
  ...
  try {
    ...
    const sections = recalculateQuoteTotals(
      existingSections.map((s, sIndex) => {
        if (sIndex !== sectionIndex) return s;

        return {
          ...s,
          lineItems: s.lineItems.filter((li) => li.id !== lineItemId),
        };
      }),
    );
    ...
    if (!updatedQuote.ok) {
      return {
        ...
        error: {
          code: ERROR_CODES.UNKNOWN,
          message: "Line item was removed but quote could not be reloaded.",
        },
      };
    }
    ...
    return {
      ...
      error: {
        code: ERROR_CODES.UNKNOWN,
        message: "Could not remove line item.",
      },
    };
  }
}
