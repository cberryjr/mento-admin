...
export function QuotePreview({ payload, editorHref }: QuotePreviewProps) {
  const {
    quoteNumber,
    title,
    status,
        ...
        <div className="space-y-8 p-8">
          <header className="flex items-start justify-between border-b border-zinc-200 pb-6">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold text-zinc-900">QUOTE</h1>
            </div>
            <div className="text-right">
              ...
              <p className="text-sm text-zinc-500">
                Prepared {formatDate(preparedAt)}
              </p>
            </div>
          </header>

          <section aria-label="Quote title">
          ...
          <section aria-label="Client details" className="space-y-1">
            ...
            {clientContact.name ? (
              <p className="text-sm text-zinc-600">{clientContact.name}</p>
            ) : null}
            {clientContact.email ? (
              <p className="text-sm text-zinc-600">{clientContact.email}</p>
            ) : null}
            {clientContact.phone ? (
              <p className="text-sm text-zinc-600">{clientContact.phone}</p>
            ) : null}
          </section>
          ...
          {sections.map((section) => (
            <section
              key={section.id}
              aria-label={section.title || "Quote section"}
              className="space-y-3"
            >
